...
class TestAccountStatementImportSheetFile(common.TransactionCase):
    ...
    def _data_file(self, filename, encoding=None):
        ...
        with open(self._get_file_path(filename), mode) as file:
            data = file.read()
            if encoding:
                data = data.encode(encoding)
            return b64encode(data)

    def test_available_import_formats(self):
        self.assertIn(
            "TXT/CSV/XLSX",
            self.AccountJournal._get_bank_statements_available_import_formats(),
        )
    ...
    def test_import_csv_file_without_extension(self):
        wizard = self.AccountStatementImport.with_context(
            journal_id=self.journal.id, account_statement_import_sheet_file_test=True
        ).create(
            {
                "statement_filename": "sample_statement",
                "statement_file": self._data_file("fixtures/sample_statement_en.csv"),
                "sheet_mapping_id": self.sample_statement_map.id,
            }
        )
        wizard.import_file_button()
